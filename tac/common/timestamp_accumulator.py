...
def get_accumulate_timestamp_idxs(
    ...
    """
    Processes a list of timestamps, selecting indices based on a regular time interval (dt) 
    and returns their local and global indices.

    This function is designed to handle video or data stream where frames or data points are timestamped.
    It selects the first timestamp in each dt window, considering potential dropped frames 
    or irregularities in timestamps.

    Parameters:
    - timestamps (List[float]): A sorted list of timestamps.
    - start_time (float): The start time for considering the intervals.
    - dt (float): The regular time interval for selecting timestamps.
    - eps (float): A small epsilon value added to avoid floating-point arithmetic issues. Defaults to 1e-5.
    - next_global_idx (Optional[int]): The global index from which to start considering timestamps. 
      If None, starts from 0. Defaults to 0.
    - allow_negative (bool): If False, negative global indices are ignored. Defaults to False.

    Returns:
    - Tuple[List[int], List[int], int]: A tuple containing three elements:
        - local_idxs (List[int]): The indices of selected timestamps in the original list.
        - global_idxs (List[int]): The global indices corresponding to each selected timestamp.
        - next_global_idx (int): The next global index to use for subsequent calls.

    The global index represents the count of dt intervals since the start_time. A timestamp
    might be chosen multiple times if it's the first in its dt window after accounting for 
    dropped frames or irregular intervals. The function is useful for synchronizing data points 
    or frames to a regular time grid.
    """
    local_idxs = list()
    global_idxs = list()
