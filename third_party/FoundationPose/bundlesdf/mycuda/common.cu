...
__device__ void calculateBarycentricCoordinate2DKernel(const Eigen::Matrix<float,3,2> &triangle, const Eigen::Vector2f &p, Eigen::Vector3f &w)
{
  ...
  numerator << CA, CP;
  denominator<<AB, AC;
  float num_det1 = numerator(0,0) * numerator(1,1) - numerator(0,1) * numerator(1,0);
  float den_det = denominator(0,0) * denominator(1,1) - denominator(0,1) * denominator(1,0);
  w(1) = num_det1 / den_det;
  numerator << AB, AP;
  float num_det2 = numerator(0,0) * numerator(1,1) - numerator(0,1) * numerator(1,0);
  w(2) = num_det2 / den_det;
  w(0) = 1-w(1)-w(2);
}
